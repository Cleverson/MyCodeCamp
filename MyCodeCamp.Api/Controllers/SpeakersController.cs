...
namespace MyCodeCamp.Api.Controllers
{
    ...
    public class SpeakersController : BaseController
    {
        ...
        public async Task<IActionResult> Get(string moniker, int id, bool includeTalks = false)
        {
            try
            {
                var speaker = includeTalks ? _repository.GetSpeakerWithTalks(id) : _repository.GetSpeaker(id);
                if (speaker == null) return NotFound();
                if (speaker.Camp.Moniker != moniker) return BadRequest("Speaker not in specific Camp");
